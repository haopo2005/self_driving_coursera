...
class BehaviouralPlanner:
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            ...
            wx = waypoints[wp_index][0]
            wy = waypoints[wp_index][1]

            wx2 = waypoints[wp_index+1][0]
            wy2 = waypoints[wp_index+1][1]

            '''
            wp = np.array([wx, wy])
            wp = np.dot(ego_rotation,(wp-ego))

            if wp[1] > self._lookahead:
                break

            arc_length += np.sqrt((wx-wx2)**2+(wy-wy2)**2)
            wp_index += 1
            '''
            curr_length = np.sqrt((wx - wx2)**2 + (wy - wy2)**2)
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        w_x = waypoints[i][0]
        w_y = waypoints[i][1]
        dist_tmp = np.sqrt((ego_x-w_x)**2+(ego_y-w_y)**2)

        if dist_tmp < closest_len:
            closest_len = dist_tmp
            closest_index = i
    # ------------------------------------------------------------------
